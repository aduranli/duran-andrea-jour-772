...
**Answer: This is the data that shows the aforementioned columns per state.  **
```{r}
...
**Answer: It seems that if the population remains steady, so does the GPD growth. **
```{r}
...
**Answer: This result varies much more. It very vaguely showing that more spending per pupil leads to less GPD growth. **
```{r}
...
**Answer: The spending per pupil and GDP growth is slightly negative and population change and GPD growth is slightly positive.**
...
**Answer: The true correlation is not equal to zero and the 95% confidence interval is negative, so the relationship is negative. The p-value is also very low so that means that there is a low chance it was a statistical aberration between a negative relationship between spending per pupil and population change.**
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer: Here the p-value is very high compared to last, making it weaker. This makes this relationship more likely to be a statistical aberration.**
```{r}
...
**Answer: This shows the amount the states that voted for Biden had a higher mean amount of money spent more per pupil and had a larger mean GDP growth and states that voted for Trump had a higher mean population growth. **
```{r}
state_data |>
  group_by(vote_2020) |>
  summarise(
...
**Answer: The p-value is pretty low so that means that there is a higher chance this is not a statistical aberration.**
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer: The p-value is even lower and the t value is a lot bigger (4.2 versus -1.8) so that means we have found something that is not a statistical aberration.**
```{r}
