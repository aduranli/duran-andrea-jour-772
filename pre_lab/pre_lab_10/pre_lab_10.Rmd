...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer: We are seeing zipcodes (both ones that are 5 numbers long and ones that are 11 numbers long) and their city names, the shape of the zip in terms of length and area. There is also a column for the geometry as a multipolygon datatype and the number of the object (the objectid). **
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer: this contains information on the zipcodes (their names and number identifiers) as well information on the longitude and latitude of the zipcodes. **
...
**Task** Run the following code. Describe the output in the space below. **Answer: We now we a map of Maryland zip codes on a latitude and longitudinal map.**
...
**Answer:** **The output says "To enable caching of data, set `options(tigris_use_cache = TRUE)`
...
**Task** Run the following code. Describe the output in the space below. **Answer: There are 18 columns and they state the state identifier, county number, zip code, name of the zip code, name of the county, and various information on the areas including longitudes and latitudes.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now there are 24 columns, each for the counties in Maryland. This dataset now states the information on the county number, zip code, name of the zip code, name of the county, and various information on the areas including longitudes and latitudes. **
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we see a map of Maryland's counties by the latitude and longitude.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now it is a map showing the zip codes and counties of Maryland using their latitudes and longitudes. **
...
**Task** Run the following code. Describe the output in the space below. **Answer: We now see zip codes and how many foreclosures there are in each zip code. The zip code with the most foreclosures (123) was 20877.**
...
foreclosure_zip_march_2023 <- foreclosure_zip |> 
  select(zip, march_2023) |>  arrange(desc(march_2023))
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we have the dataset of foreclosures in Maryland zip codes with longitudes and latitudes. **
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we see a map of all MD zip codes with the number of foreclosures in them and, with the colors, can see where there are the highest number of foreclosures, which it seems that zip codes in and around Prince George's County are the highest as well as some near Baltimore and in the northeast and northwest of MD.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer: the libraries have now been loaded and so has my API census key.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we see a dataset for the number of notices per county and onethat has the estimate total of population per county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This is the data on the estimate total population per county with the addition of the longitiudes and latitudes.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: I can see the two different datatsets and see that the notices data has the counties as just the name of the county whereas the population data has the county names and ", Maryland" which will prove to be a problem when we want to join the datasets.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we have renamed the names of the counties to be the same as the other dataset (removed ", Maryland" and any other white spaces). This will make it able to join the other dataset.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we have a dataset on the population per county in Maryland, the number of notices to foreclose per county, and the latitudes and longitudes of the counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we have the rates of notices of foreclosure per county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we can visualize the data on a map and the colors let us see that Charles County is the county with the highest rates of notices of foreclosure, followed by Prince George's County, Baltimore County and other, more rural counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now we see different colors for the data and see we can more easily see which counties have the highest or lowest rates of notices of foreclosure.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer: Now we can see the differences in scale more clearly so it's easier to see what rates the counties fall into.**
