...
**The new variable, selected_journalism_courses, creates a new table which allows us to organize it easier and makes it easier to access by finding it on the Environment tab.**
...
**The information in the expenditure date column is now read as an actual date and characterized as such and is now formatted by year, month, day and has dashes instead of slashes. :**
...
**The month column uses the function floor_date to make the date clear and not combine all the same months together or create invalid date objects by formatting by year, month. The floor_date makes it read by year, month, day because it rounds down to the "nearest boundary of the specified time unit," (the time unit being "month") which is the first of the month (from RStudio help section). **
...
**The "mutate" function is creating two new columns, the total number of votes and the percentage of votes to Moore. The total number of votes column is made by summing up cox, moore, lahsar, and wallace and write-ins. The percentage column is made by dividing the "moore" column by the previously made "total_votes" column (From ChatGPT). **
...
**The table is now arranged by the biggest percentage of Moore votes to lowest. And it shows that the county with the biggest percentage of Moore Votes, with the biggest percentage of Moore votes being 89.24% in Prince George's County.**
...
**This shows us the county with the least percentage of Moore votes. So with this answer we can see the county that Moore was supported by the least. This can be a useful answer if we want to find the county where Moore received the least support and possibly generate stories as to why.**
...
**toupper() function from base R accomplishes the same task of converting a string to uppercase. Mutate with toupper() uses the the dplyr package to apply the toupper() function to a column in a data frame and creates a new column in the data frame.**
...
**This makes sense to me because what it is doing is grouping by state and then summarizing the total amount paid then arranging it by highest amount paid to the state, showing that Maryland was paid the most with a total of $77,723,145.82**
...
1. Which payee spent the most and to whom?

2.  Which payee in Maryland spent the most?

3.  What was the average spent for each state?
