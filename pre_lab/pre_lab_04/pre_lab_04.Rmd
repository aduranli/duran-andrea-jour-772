...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** **There are names that are repeated versions of entries because of slight deviations in spelling. Ex. WASHINGTON CO COMMISSIONE and WASHINGTON CO COMMISSIONERS. **
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **The refinr package made slight changes to punctuation and capitalization and sometimes removing certain additions such as removing "Inc" when it changed "Melwood Horticultural Training Center, Inc." to "Melwood Horticultural Training Center" and then changed "Charles County Arts Alliance Inc" to "CHARLES COUNTY ARTS ALLIANCE."  **
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** **The refinr packaged changed "WASHINGTON ADVENTIST UNIVERSITY" to "Washington Adventist University", changed "WASHINGTON CO COMM. ACTION COUNCIL INC"	to "WASHINGTON COMM ACTION COUNCIL" and "WASHINGTON COLLEGE" to "Washington College." **
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** **There is no substantial difference made from the use of key_collision_merge. **
