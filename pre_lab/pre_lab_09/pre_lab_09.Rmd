...
**https://datawrapper.dwcdn.net/0PTik/1/**
...
md_gov_county |> 
  group_by(Fips_Code, County, Cox, Moore, Lashar, Wallace, Harding, `Write-ins`) |>
  summarise(count = sum(Cox, Moore, Lashar, Wallace, Harding)) |> 
  mutate(percent_moore = (Moore/sum(count)*100)) |> 
  arrange(desc(percent_moore))

md_gov_county <- write_csv(md_gov_county, "data/md_gov_county copy.csv") |> 
  group_by(Fips_Code, County, Cox, Moore, Lashar, Wallace, Harding, `Write-ins`) |>
  summarise(count = sum(Cox, Moore, Lashar, Wallace, Harding)) |> 
  mutate(percent_moore = (Moore/sum(count)*100))
...
**Now the map is only a map of Maryland and its counties, showing the varying degrees with which they voted for Moore. **
...
**Now there are the names of the counties for whichever you hover on and the percentage rounded to the nearest tenth. **
...
**https://datawrapper.dwcdn.net/2eoJd/1/**
