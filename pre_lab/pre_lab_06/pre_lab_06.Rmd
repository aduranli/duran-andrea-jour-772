...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer: Yes. They each have the same columns.**
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are still 24 rows. **
...
**Task** Create a new dataframe with population and voter data joined together. Add a description of what this code does to your reference notebook. **Answer: It joins both data frames and allows us to see the counties, opulation size in 2020, the year of 2022, the political parties, and the total number of registration for that year (2022).**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: This contains information on the demographic data of the maryland population including area code, county, age (pop over 18), number of people who identify as one race, number of people who identitfy as white, black, native, asian, pacific islander, other, or two or more races.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer: Prince George's County has the highest percentage of 18+ Black population (with 60.90%) and a 77.10% of Democrats.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: Howard County has the highest percentage of Asian population but the highest is only 19.37% compared to Prince George's Black popluation of 60.90%. The percentage of Democrats in Howard County is a little over half (52.57%), which is lower than Prince George's County Democrat population majority with a percentage of 77.10%**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer: The ZCTA5N column is a <dbl> number datatype.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer: The datatype is now <chr> or character datatype.**
...
Now we can join this dataframe to other zip code data where the zip code column is text, not numbers. text
