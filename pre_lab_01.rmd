...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **<chr> and <dbl>**
...
**Showing how many classes are offered in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Organized or arranged classes by biggest to smallest to find biggest.**
...
**Now we are organizing by term and department and arrange(term, department) changed it.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **we are now trying to know the total number of seats : total_seats = sum(seats)**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **We calculated the mean number of seats by term and the median, mean_seats = mean(seats), median_seats = median(seats)  **
...
**group_by(department) then adding min_seats = min(seats) max_seats = max(seats) Grouping by department, finding the highest and lowest number of seats for each department.**
...
1.  What classes are in person?

2.  What classes are hybrid?

3.  What department offers the most classes? 
