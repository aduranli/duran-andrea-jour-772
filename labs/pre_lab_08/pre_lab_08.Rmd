...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer: rows: 3822, columns: 4. **
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer: First, we take our dataset and we are making a column that shows the month for each row. Then we are grouping by month and finding the total number of calls per month and arranging the total calls by most to least. **
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A blank canvas. A gray box.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A bar graph that shows the number of calls (y) per month (x).**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: Now the bar graph shows the calls from lowest to highest calls per month.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: Now this is a column chart in which instead of being read down to up, it's left to right with the months on the left side and the number of calls on the bottom, giving it a neater appearance without changing the meaning of the chart.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: The bar chart no longer has a gray background, making it have a cleaner, minimal style.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: It changed the style of the bar chart to have a blue background, and slightly more details for the lines.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: There is now a title for the graph, the names of the axises are cleaner and there is also a label for the source (Baltimore County).**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I am seeing the total calls for each date.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I am now seeing a line chart that shows the calls for each date and we can see at a glance the highest and lowest number of calls and when those spikes and sharp decreases occurred.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: We now see the data set as it appears for every week but it is hard to see the date.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: Now we can see the date labels a little easier and we're still able to see the calls for each week.**
